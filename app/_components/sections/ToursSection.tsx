              ...
              <CardHeader>
                {tour.items[0].imageThumbnail && (
                  <div className="relative w-full h-[200px]">
                    <Image src={getFileUrl(tour.items[0].imageThumbnail)} alt={tour.items[0].name} fill className="rounded-md h-[200px]" />
                  </div>
                )}
              </CardHeader>
              <CardContent>
                ...
                <CardDescription>
                  <p dangerouslySetInnerHTML={{ __html: tour.items[0].content }} />
                </CardDescription>
              </CardContent>
              <CardFooter className="flex justify-between items-center">
                <span className="text-lg font-bold">{tour.items[0].cost}</span>
                <span className="text-lg font-bold">{dayjs(tour.items[0].startDate).format("YYYY-MM-DD")}</span>
